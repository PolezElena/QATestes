...
OVERRIDE_SYSTEM_USERS.forEach(async (overrideUser) => {
  test(`Full route for adding state panel object: "${getLoginFromOverrideUserItem(overrideUser)}"`, async ({
    page,
  }) => {
    ...
    await confPage.statePanelsPage.moveFromCardToPage();
    await confPage.statePanelsPage.createObjectSimple();
    await confPage.statePanelsPage.testObject();
  });
});
