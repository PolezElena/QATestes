...
OVERRIDE_SYSTEM_USERS.forEach(async (overrideUser) => {
  test(`Full route for adding MPEG-Dash object: "${getLoginFromOverrideUserItem(overrideUser)}"`, async ({ page }) => {
    ...
    await confPage.monitoringObjectPage.mpegDashPage.createObject();
  });
});
